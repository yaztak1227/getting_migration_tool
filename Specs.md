...
- The ranking distribution page lists cached powers, accepts either multiple cached selections or a direct power/range input, and accepts one or more kingdom IDs.
- On the ranking distribution page, a `選択を再取得` button is placed to the right of the `キャッシュ済みリスト` label and re-fetches only the currently selected cached powers from the API, then updates each cached timestamp in place.
...
- The ranking distribution chart is rendered by Chart.js and can be switched between bar and line chart types. Each entered kingdom ID is rendered as a separate chart, so three or more distribution charts can be displayed together. Its selection state is reflected in the URL path as `/ranking/<power-selection>/<kingdom-ids>` so the same cached chart can be reopened repeatedly in local server mode. Query parameters are also parsed on direct open.
- Ranking distribution text contrast prioritizes readability: heading notes, field labels, chart axis labels/ticks, and chart status text use strong foreground colors rather than muted styles.
...
- The ranking distribution page includes a button to save currently displayed charts via html2canvas. When fewer than 5 charts are displayed, it saves one PNG image. When 5 or more charts are displayed, it renders charts in groups of 5 and saves the generated PNG files together as one ZIP via JSZip.
...
- Ranking chart image export renders charts into an off-screen virtual DOM with fresh canvases before capture, avoiding stretched output from viewport-dependent chart sizing.
